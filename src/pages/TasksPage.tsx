...
const TasksPage = () => {
        ...
        allTasksForStatus.forEach(task => {
          if (task.tags && Array.isArray(task.tags)) {
            task.tags.forEach(tag => {
              const tagName = typeof tag === 'object' && tag ? (tag as any).name : tag;
              if (tagName) uniqueTags.add(tagName);
            });
          }
        });
        
        setAvailableTags(Array.from(uniqueTags) as any);
  ...
  const fetchTasks = useCallback(async (pageNum = 0) => {
    try {
      ...
      const newTasks = (response.data as any)?.content || (Array.isArray(response.data) ? response.data : []);
      ...
      setHasMore((response.data as any)?.last === false);
      setPage(pageNum);
  ...
  const priorityOptions = [
    { label: 'Todas las Amenazas', value: 'ALL' },
    { label: 'Amenaza Alta', value: 'HIGH' },
    { label: 'Amenaza Media', value: 'MEDIUM' },
    { label: 'Amenaza Baja', value: 'LOW' }
  ];

  const statusOptions = [
    { label: 'Misiones Activas', value: 'OPEN' },
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-black" style={{ color: 'var(--text-primary)' }}>Misiones Activas</h2>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-8 bg-[var(--surface-base)] p-5 rounded-2xl border border-[var(--border-subtle)] shadow-lg relative z-50">
        ...
        <CustomSelect 
          ...
          placeholder="Todas las Amenazas"
        />
      ...
      {tasks.length === 0 ? (
          ...
          {searchQuery || priorityFilter !== 'ALL' || tagFilter !== 'ALL' || statusFilter !== 'OPEN' ? (
            <>
              ...
              <p className="font-bold text-lg mb-1" style={{ color: 'var(--text-primary)' }}>
                Ninguna misión coincide con tus filtros
              </p>
            ...
            <>
              ...
              <p className="text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>
                Crea tu primera misión para empezar a ganar XP y acumular días de racha.
              </p>
              <button
                onClick={() => handleOpenModal(null)}
        ...
        <motion.div layout className="flex flex-col gap-3 relative z-0">
          ...
          {hasMore && (
            ...
            >
              {isLoadingMore ? '⏳ Buscando en los Archivos...' : '📜 Revelar Más Misiones'}
            </motion.button>
          )}
        </motion.div>
      )}
